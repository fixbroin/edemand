...
export const placeOrderApi = async ({
  ...
  try {
    ...
    if (lock_id) {
      formData.append("lock_id", lock_id);
    }
...
export const GetAddressCustomFieldsApi = async ({ address_id = "" } = {}) => {
  try {
    ...
    const response = await api.post(apiEndPoints.getAddressCustomFields, formData);
    return response?.data;
  } catch (error) {
...
// 27b. release slot lock api
export const releaseSlotLockApi = async ({ lock_id = "" }) => {
  try {
    const formData = new FormData();
    formData.append("lock_id", lock_id);
    const response = await api.post(apiEndPoints.releaseSlotLock, formData);
    return response?.data;
  } catch (error) {
    console.error("Error in release_slot_lock:", error);
    throw error;
  }
};
...
export const changeOrderStatusApi = async ({
  ...
  try {
    ...
    formData.append("time", time);
    if (cancel_reason_id) {
      formData.append("cancel_reason_id", cancel_reason_id);
    }
    if (additional_info) {
...
export const fetchMyCustomJobRequestsApi = async ({
  id = "",
  offset,
  limit,
  sort = "",
...
export const cancelCustomJobReqApi = async ({
  custom_job_request_id,
  cancel_reason_id = "",
  additional_info = "",
}) => {
...
// 48b. get categories hierarchical api
export const getCategoriesHierarchicalApi = async () => {
  try {
    const response = await api.post(apiEndPoints.allCategoriesHierarchical);
    if (response.status === 401) {
      toast.error("Something Went Wrong");
      return false;
    }
    ...
    console.error("Error in getCategoriesHierarchicalApi:", error);
    throw error;
  }
};
...
// 51b. get reasons api (typed) — used for cancel reasons etc.
export const getReasonsApi = async ({
  ...
  try {
    ...
    const response = await api.post(apiEndPoints.getReasons, formData);
    return response?.data;
  } catch (error) {
