...
import CancelReasonModal from "@/components/ReUseableComponents/Dialogs/CancelReasonModal";
...
const ServiceRequestDetails = () => {
  ...
  const [showCancelDialog, setShowCancelDialog] = useState(false); // State for cancel reason modal
  ...
  const handleCanceleBooking = async (e) => {
    ...
    try {
      ...
      const hasReasons = response?.data?.length > 0;
      if (hasReasons) {
  ...
  // Actually cancel the request after user picks reason
  const confirmCancelBooking = async ({ cancel_reason_id, additional_info } = {}) => {
    try {
  ...
  return (
    <Layout>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="col-span-1 lg:col-span-9">
              ...
              <div className="flex flex-col gap-6">
                ...
                {/* Reference Files */}
                {Array.isArray(serviceData?.files) && serviceData?.files.length > 0 && (() => {
                  const classifyFile = (url = "") => {
                    if (/\.(jpe?g|png|gif|webp|svg|bmp|avif)(\?.*)?$/i.test(url)) return "image";
                    if (/\.(mp4|webm|ogg|mov|m4v|mkv)(\?.*)?$/i.test(url)) return "video";
                    return "file";
                  };
                  const mediaItems = serviceData.files.filter(
                    (f) => classifyFile(f) !== "file"
                  );
                  const docItems = serviceData.files.filter(
                    (f) => classifyFile(f) === "file"
                  );
                  ...
                  return (
                    <div>
                      <h3 className="md:text-lg font-semibold mb-2 capitalize">
                        {t("referenceFiles") || "Reference Files"}
                      </h3>
                      {mediaItems.length > 0 && (
                          ...
                          {mediaItems.map((file, idx) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={onClick}
                                className="relative w-20 aspect-square rounded-md overflow-hidden border focus:outline-none focus:ring-2 focus:ring-primary/30"
                              >
                                {kind === "video" ? (
                                  <>
                                    <video
                                    ...
                                    <span className="absolute inset-0 flex items-center justify-center bg-black/40">
                                      <BsPlayCircleFill size={26} className="text-white" />
                                    </span>
                                  </>
                                ) : (
                                  <CustomImageTag
                      ...
                      {docItems.length > 0 && (
                        <ul className="flex flex-col gap-2 mt-3">
                          {docItems.map((file, idx) => {
                            const name = file.split("/").pop()?.split("?")[0] || `file-${idx + 1}`;
                            return (
                              <li key={`doc-${idx}`}>
                                <button
                                  type="button"
                                  onClick={() => handleDownload(file)}
                                  className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-md border background_color text-sm hover:border-primary transition"
                                >
                                  <span className="flex items-center gap-2 min-w-0">
                                    <MdInsertDriveFile className="primary_text_color shrink-0" size={18} />
                                    <span className="truncate">{name}</span>
                                  </span>
                                  <MdDownload className="description_color shrink-0" size={18} />
                                </button>
                              </li>
                            );
                          })}
                ...
                {serviceData?.status === "cancelled" &&
                  (serviceData?.cancel_reason || serviceData?.cancel_additional_info) && (
                    <div
                      className="flex items-start gap-3 p-4 rounded-lg border"
                      style={{
                        backgroundColor: "var(--status-cancelled-bg, rgba(220, 38, 38, 0.08))",
                        borderColor: "var(--status-cancelled, rgba(220, 38, 38, 0.4))",
                      }}
                    >
                      <MdCancel
                        size={22}
                        className="min-w-[22px] mt-1"
                        style={{ color: "var(--status-cancelled, rgb(220, 38, 38))" }}
                        aria-hidden="true"
                      />
                      <div className="flex flex-col gap-1 flex-1">
                        ...
                        {serviceData?.cancel_reason && (
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                            <span className="description_color text-sm">
                              {t("reason")}:
                            </span>
                            <span className="text-base font-medium break-words">
                              {serviceData?.translated_cancel_reason ||
                                serviceData?.cancel_reason}
                            </span>
                          </div>
                        )}
                        {serviceData?.cancel_additional_info && (
                          <div className="flex flex-col sm:flex-row gap-1 sm:gap-2">
                            <span className="description_color text-sm sm:mt-[2px]">
                              {t("additionalInfo")}:
                            </span>
                            <span className="text-sm break-words description_color">
                              {serviceData?.cancel_additional_info}
                            </span>
                          </div>
                        )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 light_bg_color p-4 rounded-lg">
                  ...
                  <div>
                    <p className="description_color text-sm">{t("startOn")}</p>
                    <p className="font-medium">
                      {dayjs(
      ...
      {/* Reference Files Lightbox (images + videos) */}
      {lightboxOpen && (
        <Lightbox
          isLightboxOpen={lightboxOpen}
          images={(serviceData?.files || []).filter((f) =>
            /\.(jpe?g|png|gif|webp|svg|bmp|avif|mp4|webm|ogg|mov|m4v|mkv)(\?.*)?$/i.test(f)
          )}
