...
const ProviderServiceDetails = () => {
  ...
  const [isLongDescTruncated, setIsLongDescTruncated] = useState(false);
  const [sanitizedLongDesc, setSanitizedLongDesc] = useState("");
  ...
  useEffect(() => {
    const raw = serviceData?.translated_long_description || serviceData?.long_description;
    if (!raw) return;
    const html = DOMPurify.sanitize(raw);
    const temp = document.createElement("div");
    temp.innerHTML = html;
    const plainText = temp.textContent || temp.innerText || "";
    setSanitizedLongDesc(html);
    setIsLongDescTruncated(plainText.trim().length > 150);
            ...
            <div className="col-span-12 lg:col-span-8">
              ...
              {sanitizedLongDesc && (
                <div className="flex flex-col">
                  ...
                  <div className="mt-4">
                    <div
                      className={`text-sm description_color leading-relaxed overflow-hidden ${!isLongDescExpanded && isLongDescTruncated ? "line-clamp-4" : ""}`}
                    ...
                    {isLongDescTruncated && (
                      ...
                      >
                        {isLongDescExpanded ? t("viewLess") : t("viewMore")}
                      </button>
                    )}
                  </div>
                </div>
