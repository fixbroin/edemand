...
import { validatePasswordStrength } from "@/components/auth/PasswordRequirements";
...
export const useProviderRegistration = ({
    ...
    const loginSettings = useSelector((state) => state?.settingsData?.settings?.login_settings);
    ...
    const handleFinalRegistration = async () => {
        ...
        if (!validatePasswordStrength(formData.password, loginSettings)) {
            showErrorMessage(t("passwordRequirementsNotMet"));
            return;
        }
        ...
        try {
            ...
            if (response?.error === false) {
                ...
                showErrorMessage(Array.isArray(msg) ? msg.join(", ") : msg);
            }
        } catch (error) {
