...
// Default schema markup for Fixbro
...
// Global time-ago formatter — mirrors Flutter timeago package thresholds exactly.
// t = translation function from useTranslation()
// dateString = "YYYY-MM-DD HH:MM:SS" (UTC) or any Date-parseable string
export const formatTimeAgo = (dateString, t) => {
  ...
  if (typeof dateString === 'string' && dateString.includes(' ') && !dateString.includes('T')) {
    ...
    sentDate = new Date(Date.UTC(year, month - 1, day, hours, minutes, seconds));
  } else {
    sentDate = new Date(dateString);
  }

  if (isNaN(sentDate.getTime())) return '';

  const diffInMs  = new Date().getTime() - sentDate.getTime();
  const seconds   = Math.floor(diffInMs / 1000);
  const minutes   = Math.floor(diffInMs / (1000 * 60));
  const hours     = Math.floor(diffInMs / (1000 * 60 * 60));
  const days      = Math.floor(diffInMs / (1000 * 60 * 60 * 24));

  const suffix = t("timeagoSuffixAgo");

  // Matches Flutter timeago thresholds exactly
  if (seconds < 45)   return t("timeagoLessThanOneMinute");
  if (seconds < 90)   return `${t("timeagoAboutAMinute")} ${suffix}`;
  if (minutes < 45)   return `${minutes} ${t("timeagoMinutes")} ${suffix}`;
  if (minutes < 90)   return `${t("timeagoAboutAnHour")} ${suffix}`;       // 45–89 min
  if (hours < 24)     return `${hours} ${t("timeagoHours")} ${suffix}`;    // 1.5 h → hours count
  if (hours < 48)     return `${t("timeagoADay")} ${suffix}`;
  if (days < 7)       return `${days} ${t("timeagoDays")} ${suffix}`;
  if (days < 14)      return `${t("timeagoAboutAWeek")} ${suffix}`;
  if (days < 30)      return `${Math.floor(days / 7)} ${t("timeagoWeeks")} ${suffix}`;
  if (days < 60)      return `${t("timeagoAboutAMonth")} ${suffix}`;
  if (days < 365)     return `${Math.floor(days / 30)} ${t("timeagoMonths")} ${suffix}`;
  if (days < 730)     return `${t("timeagoAboutAYear")} ${suffix}`;
  return `${Math.floor(days / 365)} ${t("timeagoYears")} ${suffix}`;
};
